...
class CartPoleSwingUpEnv(gym.Env):
    ...
    def stateUpdate(self,action,state, noise=0):
        ...
        x     += self.np_random.randn() * noise
        theta += self.np_random.randn() * noise
    ...
    def reset(self):
        ...
        self.state = self.np_random.normal(loc=np.array([0.0, 0.0, np.pi, 0.0]), scale=np.array([0.2, 0.2, 0.2, 0.2]))
